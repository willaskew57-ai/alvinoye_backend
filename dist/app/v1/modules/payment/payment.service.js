 ...
 */
export const createCheckoutSessionService = async (user, parcelId) => {
    // Find parcel
    ...
    if (!session.id)
        throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, 'Stripe session ID missing');
    if (!session.url)
        throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, 'Stripe checkout URL not generated');
 ...
 * 3️⃣ Refund Payment
 */
export const refundPaymentService = async (paymentId, reason) => {
    const payment = await Payment.findById(paymentId);
    if (!payment)
        throw new AppError(httpStatus.NOT_FOUND, 'Payment not found');
    if (payment.status !== PAYMENT_STATUS.SUCCESS) {
        throw new AppError(httpStatus.BAD_REQUEST, 'Only successful payments can be refunded');
    }
