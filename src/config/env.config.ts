...
const configs = {
  ...
  wallet_default_commission_percent:
    parseFloat(process.env.WALLET_DEFAULT_COMMISSION_PERCENT || '10') || 10,
  // Cron expression for the weekly payout distribution (pending -> available).
  // Default: every Sunday at 00:00.
  wallet_distribution_cron:
    process.env.WALLET_DISTRIBUTION_CRON || '0 0 * * 0',
