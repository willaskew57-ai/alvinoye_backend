...
const createRefundRequest = async (
  ...
  if (!parcel) {
    throw new AppError(
      ...
      'Parcel not found or unauthorized'
    );
  }
  ...
  if (!payment) {
    throw new AppError(
      ...
      'No successful payment found for this parcel'
    );
  }
  ...
  if (existingRequest) {
    throw new AppError(
      httpStatus.CONFLICT,
      'A refund request already exists for this parcel'
    );
  }
...
const processRefundDecision = async (
  requestId: string,
