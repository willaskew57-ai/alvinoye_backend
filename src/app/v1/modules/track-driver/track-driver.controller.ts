...
const getDriverLocation = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await TrackDriverServices.getDriverLocationFromDB(driverId as string);
 ...
 * @access Private
 */
const getLocationHistory = catchAsync(async (req: Request, res: Response) => {
  const { driverId } = req.params;
  const limit = req.query.limit ? parseInt(req.query.limit as string) : 50;

  const result = await TrackDriverServices.getLocationHistoryFromDB(
    driverId as string,
    limit
  );
 ...
 * @access Private
 */
const getParcelDriverLocation = catchAsync(
  async (req: Request, res: Response) => {
    const { parcelId } = req.params;
    const result = await TrackDriverServices.getParcelDriverLocationFromDB(
      parcelId as string
    );
 ...
 * @access Private - Driver only
 */
const markOffline = catchAsync(async (req: Request, res: Response) => {
  const driverId = req.user?.user_id;
...
const getNearbyDrivers = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await TrackDriverServices.getNearbyDriversFromDB(
